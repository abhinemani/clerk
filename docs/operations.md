...
docker run --rm -v clerk-data:/data -v "$PWD/backups:/backups" alpine \
  tar czf "/backups/clerk-$(date +%F).tar.gz" -C /data .
...
docker run --rm -v clerk-data:/data -v "$PWD/backups:/backups" alpine \
  tar xzf /backups/clerk-YYYY-MM-DD.tar.gz -C /data
docker compose up -d
```
...
```bash
pg_dump "$DATABASE_URL" --format=custom --file="clerk-$(date +%F).dump"
# restore: pg_restore --clean --dbname="$DATABASE_URL" clerk-YYYY-MM-DD.dump
```
...
- **S3/MinIO:** set `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`,
  ...
  relative paths as keys (`mc mirror /data/blobs alias/clerk-blobs`), then set
