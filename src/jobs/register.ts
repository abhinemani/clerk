...
export function registerJobs(): void {
  ...
  const sweep = async () => {
    ...
    // into plays. Full rebuild nightly — a materialized aggregate of the
    // append-only record, so it can never drift from what actually happened.
    try {
      const [{ getRepository }, { rebuildAgencyPlays }, { defaultDeps }] = await Promise.all([
        import("@/db/createRepository"),
      ...
      const repo = await getRepository();
      for (const agency of await repo.listAgencies()) {
        await rebuildAgencyPlays(defaultDeps(repo), agency.id);
      }
    } catch (err) {
